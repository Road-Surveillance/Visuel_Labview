...
class Detection:
    ...
    def yolo_detection(self, image, classes=None, agnostic_nms=True, max_det=100):
        ...
        detections = non_max_suppression(
            ...
            multi_label=False,  # Simplification
            max_det=max_det,
        )
    ...
    # Reste du code original conservé identique
    def ResizeImg(self, img, size):
        h1, w1, _ = img.shape
        h, w = size
        if w1 < h1 * (w / h):
            img_rs = cv2.resize(img, (int(float(w1 / h1) * h), h), interpolation=cv2.INTER_AREA)
            ...
            img_rs = cv2.resize(img, (w, int(float(h1 / w1) * w)), interpolation=cv2.INTER_AREA)
    ...
    def load_model(self, path, train=False):
        # Charger le modèle avec less de surcharge
        ...
        results_list.append(
            {
               "label": str(label),
                ...
                "crop_path": str(crop_filename),
                "detection_type": str(det_type),
                "vehicle_color": str(detected_color)
            }
        )
